...
The `posterior` function, when provided with a character vector of model names, can also be used to combined outputs from multiple models, possibly in different directories. This is achieved by reading the `*.rds` files created by `stanSave()` directly. In this case the `pars` argument must be specified. These can then also be plotted or printed for model comparison purposes.

...
#' @export
# method
"histplot.stanPosterior" <- function(object) {
...
#' @export
# method
"histplot.stanPosteriors" <- function(object) {
    ...
    gg <- ggplot(dfr) + 
        geom_histogram(aes(x = value, fill = model), bins = 50) +
        facet_wrap(~label, scales = "free") +
...
#' @export
# method
"histplot.stanOutput" <- function(object) {
